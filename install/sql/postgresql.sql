...
CREATE TABLE users(id SERIAL, gid integer REFERENCES groups(id) ON DELETE CASCADE, nick varchar(255), password varchar(32), name varchar(255), lastname varchar(255), birthday timestamp without time zone, gender boolean, email varchar(128), show_email boolean, im varchar(255), show_im boolean, country varchar(255), city varchar(255), photo varchar(512), register_date timestamp without time zone, last_visit timestamp without time zone, captcha integer, blocks varchar(255), additional text, raw_additional text, news_on_page integer, comments_on_page integer, threads_on_page integer, show_avatars boolean, show_ua boolean, show_resp boolean, theme integer REFERENCES themes(id) ON DELETE CASCADE, gmt varchar(3), filters varchar(512), mark integer REFERENCES marks(id) ON DELETE CASCADE, banned boolean, sort_to boolean, openid varchar(255), PRIMARY KEY(id), UNIQUE(nick));
...
CREATE TABLE sessions(id SERIAL, session_id varchar(128), uid integer REFERENCES users(id) ON DELETE CASCADE, tid integer REFERENCES threads(id) ON DELETE CASCADE, timest timestamp without time zone, PRIMARY KEY(id), UNIQUE(session_id));
