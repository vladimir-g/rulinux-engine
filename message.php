	...
	//{
		...
		if($pages > 1)
		{
			...
			if (isset($_GET['all']))
                ...
                $baseQuery = '
                    SELECT c.subject subj, c.cid, c.tid, c.uid, c.referer ref, u.status, c.ip IP, c.deleted, u.status active, c.UserAgent UA, c.deleted_for, u.nick, c.timestamp, u.showUA, c.comment, u.health
		...
		if ($show_hid == 1)
		{
			...
			{
				if (isset($_GET['all']))
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC', 'assoc_array');
				else
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC LIMIT '.$limit, 'assoc_array');
			}
			else
			{
				if (isset($_GET['all']))
                                    $comment_ids0 = $baseC->other_query($baseQuery.$add.' AND tid='.$id.' AND fid='.$fid.' ORDER BY cid ASC', 'assoc_array');
				else
                                    $comment_ids0 = $baseC->other_query($baseQuery.$add.' AND tid='.$id.' AND fid='.$fid.' ORDER BY cid ASC LIMIT '.$limit, 'assoc_array');
			}
		}
		else
		{
			...
			{
				if (isset($_GET['all']))
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND `deleted`=0  AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC', 'assoc_array');
				else
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND `deleted`=0  AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC LIMIT '.$limit, 'assoc_array');
			}
			else
			{
				if (isset($_GET['all']))
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND fid='.$fid.' AND `deleted`=0'.$add.' ORDER BY cid ASC', 'assoc_array');
				else
                                    $comment_ids0 = $baseC->other_query($baseQuery.' AND tid='.$id.' AND fid='.$fid.' AND `deleted`=0'.$add.' ORDER BY cid ASC LIMIT '.$limit, 'assoc_array');
			}
		}
//		if ($show_hid == 1)
//		{
...
//			{
//				if (isset($_GET['all']))
//                                $baseC->other_query($baseQuery.' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC')
//					$comment_ids0 = $baseC->oth('comments', 'cid', null, 'tid', $id, ' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC');
//				else
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, ' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC LIMIT '.$limit);
//			}
//			else
//			{
//				if (isset($_GET['all']))
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, $add.' AND fid='.$fid.' ORDER BY cid ASC');
//				else
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, $add.' AND fid='.$fid.' ORDER BY cid ASC LIMIT '.$limit);
//			}
//		}
//		else
//		{
...
//			{
//				if (isset($_GET['all']))
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, 'AND `deleted`=0  AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC');
//				else
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, 'AND `deleted`=0  AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC LIMIT '.$limit);
//			}
//			else
//			{
//				if (isset($_GET['all']))
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, ' AND fid='.$fid.' AND `deleted`=0'.$add.' ORDER BY cid ASC');
//				else
//					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, ' AND fid='.$fid.' AND `deleted`=0'.$add.' ORDER BY cid ASC LIMIT '.$limit);
//			}
//		}
		...
		if (strlen($comment_ids[0]) > 0)
		{
			foreach ($comment_ids as $comment_id)
			{
				...
				$deleted = '';
                                $tmp = $comment_id;
				//$tmp['subj'] = $baseC->get_field_by_id('comments', 'subject', $comment_id, 'cid');
				//$tmp['uid'] = $baseC->get_field_by_id('comments', 'uid', $comment_id, 'cid');
				//$tmp['ref'] = $baseC->get_field_by_id('comments', 'referer', $comment_id, 'cid');
				//$tmp['active'] = $baseC->get_field_by_id('users', 'status',  $tmp['uid'], 'id');
				//$tmp['IP'] = $baseC->get_field_by_id('comments', 'IP', $comment_id, 'cid');
				//$tmp['UA'] = htmlspecialchars($baseC->get_field_by_id('comments', 'UserAgent', $comment_id, 'cid'));
				if($baseC->get_field_by_id('comments', 'deleted', $comment_id, 'cid') == 1)
				{
					$tmp['deleted_for'] = '';
					//$tmp['deleted_for'] = $baseC->get_field_by_id('comments', 'deleted_for', $comment_id, 'cid');
					//$tmp['deleted_by'] = $baseC->get_field_by_id('users', 'nick', $baseC->get_field_by_id('comments', 'deleted_by', $comment_id, 'cid'));
					$deleted = '<span style="font-weight:bolder;">Скрыто '.$tmp['deleted_by'].' по причине "'.$tmp['deleted_for'].'"</span><br>';
				}
				else
					$deleted = '';
				//$tmp['timestamp'] = $baseC->get_field_by_id('comments', 'timestamp', $comment_id, 'cid');
				$tmp['timestamp'] = $baseC->timeToSTDate($tmp['timestamp']);
				$tmp['showUA'] = $baseC->get_field_by_id('users', 'showUA', $tmp['uid']);
				//$tmp['comment'] = $baseC->get_field_by_id('comments', 'comment', $comment_id, 'cid');
				$tmp['comment'] = str_replace("\n", "<br>\n", $tmp['comment']);
				$answtoauthor = $usersC->get_user_info($baseC->get_field_by_id('comments', 'uid', $tmp['ref'], 'cid'));
				$comment_author = $usersC->get_user_info($baseC->get_field_by_id('comments', 'uid', $comment_id['cid'], 'cid'));
				//$user = $tmp['uid'] <= 0 ? 'anonymous' : $baseC->get_field_by_id('users', 'nick', $tmp['uid']);
				...
				else
				{
					...
					$count = $baseC->other_query("SELECT count(cid) FROM comments WHERE tid = $id AND cid <= ".$comment_id['cid']);
					$pageN = floor($count[0][0]/$perpage);
					?>
					<div class="messages">
					<div class="comment" id="cmm<?=$comment_id['cid']?>">
					<div class=title><?print $reason?>
					<a href="message.php?newsid=<?=$id?>&fid=<?=$fid?>&page=<?=$pageN?>#<?=$comment_id['cid']?>"><img border="0" src="design/<?=$tpl_name ?>/id.png" alt="[#]"></a>
					<?
					$del = $baseC->get_field_by_id('comments', 'deleted', $comment_id['cid'], 'cid');
					$usrname = $baseC->eread('users', 'nick', null, 'id', $baseC->eread('comments', 'uid', null, 'cid', $comment_id['cid']));
					if($_SESSION['user_admin'])
					{
						?>
						...
						{
							?>
							<a href="message.php?newsid=<?print $id?>&del=<?print $comment_id['cid']?>"><img border="0" src="design/<?=$tpl_name ?>/hide.png" alt="[Скрыть]"></a>
							<?
						}
						else
						{
							?>
							<a href="message.php?newsid=<?print $id?>&res=<?print $comment_id['cid']?>"><img border="0" src="design/<?=$tpl_name ?>/unhide.png" alt="[Восстановить]"></a>
							<?
						}
						?>
						<a href="edit-message.php?cid=<?print $comment_id['cid']?>"><img border="0" src="design/<?=$tpl_name ?>/edit.png" alt="[Редактировать]"></a>
						<a href="message.php?newsid=<?print $id?>&rem=<?print $comment_id['cid']?>"><img border="0" src="design/<?=$tpl_name ?>/remove.png" alt="[Удалить]"></a>
						<?
					}
					else if ($usrname == $_SESSION['user_name'])
					{
					...
					<div class=msg id=<?=$comment_id['cid']?>>
					...
					<div class=reply>[<a href="comment.php?answerto=<?=$id?>&cid=<?=$comment_id['cid']?>&fid=<?=$fid?>&news">Ответить на это сообщение</a>]
