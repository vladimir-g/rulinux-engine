...
if (intval($_GET['newsid'])>0) //Тело
{
	...
	//{
		...
		if ($show_hid == 1)
		{
			...
			{
				if (isset($_GET['all']))
					$comment_ids0 = $baseC->eread('comments', 'cid', null, 'tid', $id, ' AND fid='.$fid.' AND uid > 0'.$add.' ORDER BY cid ASC');
				else
		...
		if ($comment_ids[0] != '')
		{
			foreach ($comment_ids as $comment_id)
			{
				...
				$comment_author = $usersC->get_user_info($baseC->get_field_by_id('comments', 'uid', $comment_id, 'cid'));
				$user = $tmp['uid'] <= 0 ? 'anonymous' : $baseC->get_field_by_id('users', 'nick', $tmp['uid']);
				$user_active = $tmp['uid'] <= 0 ? 'anonymous' : $baseC->get_field_by_id('users', 'nick', $tmp['uid']);
				...
				else
				{
					...
					$count = $baseC->other_query("SELECT count(cid) FROM comments WHERE tid = $id AND cid <= $comment_id");
					$pageN = floor($count[0][0]/$perpage);
					?>
					<div class="messages">
					<div class="comment" id="cmm<?=$comment_id?>">
					<div class=title><?print $reason?>
					<a href="message.php?newsid=<?=$id?>&fid=<?=$fid?>&page=<?=$pageN?>#<?=$comment_id?>"><img border="0" src="design/<?=$tpl_name ?>/id.png" alt="[#]"></a>
					<?
					$del = $baseC->get_field_by_id('comments', 'deleted', $comment_id, 'cid');
					$usrname = $baseC->eread('users', 'nick', null, 'id', $baseC->eread('comments', 'uid', null, 'cid', $comment_id));
					if($_SESSION['user_admin'])
					{
						?>
						...
						{
							?>
							<a href="message.php?newsid=<?print $id?>&del=<?print $comment_id?>"><img border="0" src="design/<?=$tpl_name ?>/hide.png" alt="[Скрыть]"></a>
							<?
						}
						else
						{
							?>
							<a href="message.php?newsid=<?print $id?>&res=<?print $comment_id?>"><img border="0" src="design/<?=$tpl_name ?>/unhide.png" alt="[Восстановить]"></a>
							<?
						}
						?>
						<a href="edit-message.php?cid=<?print $comment_id?>"><img border="0" src="design/<?=$tpl_name ?>/edit.png" alt="[Редактировать]"></a>
						<a href="message.php?newsid=<?print $id?>&rem=<?print $comment_id?>"><img border="0" src="design/<?=$tpl_name ?>/remove.png" alt="[Удалить]"></a>
						<?
					}
					else if ($usrname == $_SESSION['user_name'])
					{
					...
					<div class=reply>[<a href="comment.php?answerto=<?=$id?>&cid=<?=$comment_id?>&fid=<?=$fid?>&news">Ответить на это сообщение</a>]
