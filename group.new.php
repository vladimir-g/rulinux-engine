...
require_once('classes/forum.new.class.php');
...
if($_GET['action']==remove)
{
	if($_SESSION['user_admin'])
	{
		...
		$tid = $_GET['tid'];
		$fid = $_GET['fid'];
		if(forumClass::deleteThread($tid))
		{
...
else if($_GET['action']==move)
{
	if($_SESSION['user_admin'])
	{
		...
		{
			$newfid= $_POST['newfid'];
			if(forumClass::moveThread($tid, $newfid))
			{
...
else if($_GET['action']==hide)
{
	if($_SESSION['user_admin'])
	{
		...
		{
			$uid = base::eread('users', 'id', '', 'nick', $_SESSION['user_name']);
			$reason = $_POST['purpose'];
			if(forumClass::hideThread($tid, $uid, $reason))
			{
...
else if($_GET['action']==attach)
{
	if($_SESSION['user_admin'])
	{
		...
		$tid = $_GET['tid'];
		$fid = $_GET['fid'];
		if(forumClass::attachThread($tid))
		{
...
else if($_GET['action']==detach)
{
	if($_SESSION['user_admin'])
	{
		...
		$tid = $_GET['tid'];
		$fid = $_GET['fid'];
		if(forumClass::detachThread($tid))
		{
...
else if($_GET['action']==unhide)
{
	if($_SESSION['user_admin'])
	{
		...
		$tid = $_GET['tid'];
		$fid = $_GET['fid'];
		if(forumClass::unhideThread($tid))
		{
...
if (isset($_GET['group'])=='') 
{
	...
	$_GET['group'] = base::eread('forums', 'forum_id', '', 'rewrite', $_GET['group']);
	if (ereg("^[0-9]*$", $_GET['group'])) 
	{
		$forumid = (int)$_GET['group'];
		$forum = base::other_query("SELECT name, description FROM forums WHERE forum_id = '$forumid'");
		...
		<?
		$sel = base::other_query("SELECT forum_id, name, rewrite FROM forums ORDER BY sort");
		foreach($sel as $val)
		{
		...
		<?
			$text = base::other_query("SELECT shortfaq FROM forums WHERE forum_id  = '$forumid'");
		?>
		<style>
		...
		$perpage = (int)$_SESSION['user_login'] > 0 ? base::eread('users', 'threads_on_page', '', 'id', $_SESSION['user_login']) : 30;
		$show_hid = $info["show_hid"];
		$sort_to = (int)$_SESSION['user_login'] > 0 ? base::eread('users', 'sort_to', '', 'id', $_SESSION['user_login']) : 0;
		if($show_hid == 1)
		{
			$threads_count = base::other_query('SELECT COUNT(tid) FROM threads WHERE fid = '.$forumid);
		}
		else
		{
			$threads_count = base::other_query('SELECT COUNT(tid) FROM threads WHERE fid = '.$forumid.' AND deleted != 1');
		}
		$fn = $forum->getThreads($forumid, $page, $perpage, $sort_to, $show_hid);
		$pages = ceil($threads_count[0][0]/$perpage);
		foreach($fn as $array)
		{
			$user = base::eread('users', 'nick', '', 'id', $array["uid"]);
			?>
			<tr>
			<?
			if($_SESSION['user_admin'])
		...
		for ($p = 1; $p <= $pages; $p++)
		{
			...
			{
				echo '<a href=/forum-'.base::eread('forums', 'rewrite', '', 'forum_id', $_GET['group']).'_'.$p.'>'.$p.'</a>&nbsp;';
			}
		}
		echo ']';
		}
