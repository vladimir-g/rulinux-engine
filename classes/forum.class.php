...
//				}
	...
	function addThread($fid, $uid, $timestamp, $subj, $message, $ip, $useragent, $raw_comment, $mconf = 0, $filthy_lang)
	{
		...
		$query = "INSERT INTO threads(tid, fid, uid, ip, posting_date, chandging_date, attached, closed, deleted, mconf) VALUES((SELECT MAX(tid) FROM comments)+1, '$fid', '$uid', '$ip', '$timestamp', '$timestamp', '0', '0', '0', $mconf)";
		$ath = mysql_query($query);
		$tid = mysql_insert_id();
		//echo $query;
		if($ath)
	...
	function editMessage($cid, $subj, $message, $raw_message, $filthy_lang)
	{
		$ret;
		$dbcnx = @mysql_connect($GLOBALS['db_host'], $GLOBALS['db_user'], $GLOBALS['db_pass']); 
		if (!$dbcnx) 
		...
		$query = "UPDATE comments SET subject = '$subj', comment='$message', raw_comment = '$raw_message', filthy_lang = '$filthy_lang' WHERE cid = '$cid'";
		$ath = mysql_query($query);
		//echo $query;
		if($ath)
		{
