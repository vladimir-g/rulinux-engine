...
class base{
	...
	function strToTeX($string)
	{
		...
		preg_match("#(\\\\begin)(\[)?(abap|actionscript|actionscript3|ada|apache|applescript|apt_sources|asm|asp|autoit|avisynth|bash|basic4gl|bf|bibtex|blitzbasic|bnf|boo|c|c_mac|caddcl|cadlisp|cfdg|cfm|cil|cmake|cobol|cpp|cpp-qt|csharp|css|d|dcs|delphi|diff|div|dos|dot|eiffel|e-mail|erlang|fo|fortran|freebasic|genero|gettext|glsl|gml|gnuplot|groovy|haskell|hq9plus|html4strict|idl|ini|inno|intercal|io|java|java5|javascript|kixtart|klonec|latex|lisp|locobasic|lolcode|lotusformulas|lotusscript|lscript|lsl2|lua|m68k|make|matlab|mirc|modula3|mpasm|mxml|mysql|nsis|oberon2|objc|ocaml|ocaml-brief|oobas|oracle11|oracle8|pascal|per|perl|php|php-brief|pic16|pixelbender|plsql|povray|powershell|progress|prolog|providex||python|qbasic|rails|rebol|reg|robots|ruby|sas|scala|scheme|scilab|sdlbasic|smalltalk|smarty|sql|tcl|teraterm|text|thinbasic|tsql|typoscript|vb|vbnet|verilog|vhdl|vim|visualfoxpro|visualprolog|whitespace|whois|winbatch|xml|xorg_conf|xpp|z80)?(\])?({highlight})(.*?[^\\\\end{highlight}]?)(\\\\end{highlight})#sim", $string, $code);
		if($code[1]!='')
		{
			$code[6] = base::highlight($code[6], $code[3], "geshi/geshi");
			$string = preg_replace("#(\\\\begin)(\[)?(abap|actionscript|actionscript3|ada|apache|applescript|apt_sources|asm|asp|autoit|avisynth|bash|basic4gl|bf|bibtex|blitzbasic|bnf|boo|c|c_mac|caddcl|cadlisp|cfdg|cfm|cil|cmake|cobol|cpp|cpp-qt|csharp|css|d|dcs|delphi|diff|div|dos|dot|eiffel|e-mail|erlang|fo|fortran|freebasic|genero|gettext|glsl|gml|gnuplot|groovy|haskell|hq9plus|html4strict|idl|ini|inno|intercal|io|java|java5|javascript|kixtart|klonec|latex|lisp|locobasic|lolcode|lotusformulas|lotusscript|lscript|lsl2|lua|m68k|make|matlab|mirc|modula3|mpasm|mxml|mysql|nsis|oberon2|objc|ocaml|ocaml-brief|oobas|oracle11|oracle8|pascal|per|perl|php|php-brief|pic16|pixelbender|plsql|povray|powershell|progress|prolog|providex||python|qbasic|rails|rebol|reg|robots|ruby|sas|scala|scheme|scilab|sdlbasic|smalltalk|smarty|sql|tcl|teraterm|text|thinbasic|tsql|typoscript|vb|vbnet|verilog|vhdl|vim|visualfoxpro|visualprolog|whitespace|whois|winbatch|xml|xorg_conf|xpp|z80)?(\])?({highlight})(.*?[^\\\\end{highlight}]?)(\\\\end{highlight})#sim", '<fieldset><legend>'.$code[3].'</legend>'.$code[6].'</fieldset>', $string);
		}
		preg_match("#(\\\\begin{)(flushleft|flushright|center)(})(.*?[^\\\\end{flushleft|flushright|center}]?)(\\\\end{)(flushleft|flushright|center)(})#sim", $string, $flush);
		$flush[4] = str_replace("\n", "", $flush[4]);
		$flush[4] = str_replace("\\", "<br id=\"br\">", $flush[4]);
		$string = preg_replace("#(\\\\begin{flushleft})(.*?[^\\\\end{flushleft}]?)(\\\\end{flushleft})#sim",'<p align=\"left\">'.$flush[4].'</p id=\"end{flushleft}\">', $string);
		$string = preg_replace("#(\\\\begin{flushright})(.*?[^\\\\end{flushright}]?)(\\\\end{flushright})#sim",'<p align=\"right\">'.$flush[4].'</p id=\"end{flushright}\">', $string);
		$string = preg_replace("#(\\\\begin{center})(.*?[^\\\\end{center}]?)(\\\\end{center})#sim",'<p align=\"center\">'.$flush[4].'</p id=\"end{center}\">', $string);
		preg_match("#(\\\\b{)(.*?[^}]?)(})#sim", $string, $bold);
		$string = preg_replace("#(\\\\b{)(.*?[^}]?)(})#sim",'<b>'.$bold[2].'</b>', $string);
		preg_match("#(\\\\i{)(.*?[^}]?)(})#sim", $string, $italic);
		$string = preg_replace("#(\\\\i{)(.*?[^}]?)(})#sim",'<i>'.$italic[2].'</i>', $string);
		preg_match("#(\\\\u{)(.*?[^}]?)(})#sim", $string, $u);
		$string = preg_replace("#(\\\\u{)(.*?[^}]?)(})#sim",'<u>'.$u[2].'</u>', $string);
		preg_match("#(\\\\s{)(.*?[^}]?)(})#sim", $string, $s);
		$string = preg_replace("#(\\\\s{)(.*?[^}]?)(})#sim",'<s>'.$s[2].'</s>', $string);
		preg_match("#(\\\\sub{)(.*?[^}]?)(})#sim", $string, $sub);
		$string = preg_replace("#(\\\\sub{)(.*?[^}]?)(})#sim",'<sub>'.$sub[2].'</sub>', $string);
		preg_match("#(\\\\sup{)(.*?[^}]?)(})#sim", $string, $sup);
		$string = preg_replace("#(\\\\sup{)(.*?[^}]?)(})#sim",'<sup>'.$sup[2].'</sup>', $string);
		$tags = array
		(
		...
		foreach ($tags as $tag => $val)
		{
			...
			if ($tag == 'code')
			{
				preg_match($re.'U', $string, $match);
				$string = preg_replace($re.'U', "$val$2</ol></fieldset>", $string);
				$with_breaks = preg_replace('/\n/', '<li style="background-color:#000000; padding-left: 5px; color: gray">&nbsp;', $match[2]);
            			$string = str_replace($match[2], $with_breaks, $string);
			}
			if ($tag == 'list')
			{
				preg_match($re.'U', $string, $match);
				$string = preg_replace($re.'U', "$val$2</ul>", $string);
			...
			{
				preg_match($re.'U', $string, $match);
				$string = preg_replace($re.'U', "$val$2</ol>", $string);
				$with_breaks = str_replace('{*}', '<li>&nbsp;', $match[2]);
           			$string = str_replace($match[2], $with_breaks, $string);
			}
		}
		preg_match("#(\r\n\r\n|<p>)(>|&gt;)(.*?[^\n]?)(\n)#sim", $string, $quote);
		$string = preg_replace("#(\r\n\r\n|<p>)(>|&gt;)(.*?[^\n]?)(\n)#sim","$quote[1]<i>>$quote[3]</i>\r\n\r\n", $string);
